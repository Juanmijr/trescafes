...
function cargarImagenes() {
    ...
    imagenes[cartas].src = "img/juegoJorge/" + cartas + ".png";
    cartas++;
    if (cartas < 6) {
...
function iniciar() {
    cartas = 0;
    errores = 0;
    canvas = document.getElementById("canvasJorge");
    canvas.width = 630;
    canvas.height = 510;
...
function volteaCarta() {
    ...
    if (cartas < 6) {
        ...
        ctx.fillStyle = "white";
        ctx.fillText("JUEGO TERMINADO", 130, canvas.height / 2);
    }
}
...
function actualizar() {
    ...
    ctx.font = "1.440rem Lato";
    ctx.fillStyle = "black";
    ctx.fillText("Aciertos: " + String(cartas), canvas.width - 120, 505);
    ctx.fillText("Errores: " + String(errores), canvas.width - 620, 505);
    ctx.restore();
}