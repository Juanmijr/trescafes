...
var fichas_O = 0;
var largo = 120;
var colorTablero = "#e3a364";
var colorCanvas = "black";

window.onload = cargarImagenes;

function cargarImagenes() {
    imagenes[nImagen] = new Image();
    imagenes[nImagen].src = "img/juegoLolo/" + nImagen + ".png";
    nImagen++;
    if (nImagen < 2) {
        imagenes[nImagen - 1].onload = cargarImagenes;
    } else {
        imagenes[nImagen - 1].onload = iniciar;
    }
}
...
function crearTablero() {
    ...
    ctx.strokeStyle = colorTablero;
    ctx.lineWidth = 20;

    /* Dibujar las lineas del tablero del 3 en raya */
    ctx.beginPath();
    ctx.moveTo(330, 20);
    ctx.lineTo(330, 420);
    ctx.stroke();
...
function pintarX(ficha) {
    ctx.drawImage(imagenes[1], ficha.x, ficha.y, ficha.w, ficha.h);
}

function pintarO(ficha) {
    ctx.drawImage(imagenes[0], ficha.x, ficha.y, ficha.w, ficha.h);
}
...
function tiraMaquina() {
    ...
    if (gameOver == false) {
        ...
        eleccion = Math.floor(Math.random() * (posibilidades.length - 1));
        ii = posibilidades[eleccion];
